...
class LightningPricePredictor(L.LightningModule):
    ...
    def compute_loss(
        ...
        pred_val, pred_year, pred_month, pred_day, pred_sec, pred_stock = model_outputs
        true_val, true_year, true_month, true_day, true_sec, true_stock = stock_info
        ...
        sec_loss = self.sector_loss(pred_sec, true_sec)
        stock_loss = self.stock_loss(pred_stock, true_stock)
        return [price_loss, year_loss, month_loss, day_loss, sec_loss, stock_loss]
    ...
    def training_step(self, batch, batch_indx):
        ...
        loss = self.compute_loss(
            model_outputs,
            (true_val, input[-5], input[-4], input[-3], input[-2], input[-1]),
            random_mask,
        )
        ...
        self.log("train_stock", loss[5], prog_bar=False, on_epoch=True, on_step=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.compute_loss(
            model_outputs, (true_val, input[-5], input[-4], input[-3], input[-2], input[-1])
        )
        ...
        self.log("val_stock", loss[5], on_epoch=True, on_step=True, logger=True)
...
class LightningMulitClassPricePredictor(LightningPricePredictor):
    def __init__(self, num_sector, num_stock: int, **kwargs: LightningPricePredictor):
        super().__init__(**kwargs)
        self.num_sector = num_sector
        self.num_stock = num_stock
        self.price_predictor = MulitClassPricePredictor(num_sector, num_stock, **kwargs)
