...
def sanitize_table_name(ticker: str, used: set[str]) -> str:
    ...
    character replaced by ``_``. If that name collides with one already in
    ``used`` (compared case-insensitively, since SQLite identifiers are
    case-insensitive), a numeric suffix (``_2``, ``_3``, …) is appended until
    it is unique. The chosen name is added to ``used``.
